...
def _role_based_redirect(request):
    """Return a redirect response based on user role if applicable, else None.

    Rules:
    - Super admins (is_superuser or email in SUPER_ADMIN_EMAILS) → super admin dashboard
    - Active canteen staff → their canteen dashboard for assigned college
    - Otherwise → None (caller can render normal page)
    """
    try:
        if not request.user.is_authenticated:
            return None
        ...
        if request.user.is_superuser or user_email in super_admin_emails:
...
def home(request):
    """Home page; role-based redirect for privileged users on root URL"""
    role_redirect = _role_based_redirect(request)
    if role_redirect:
...
def oauth_complete(request):
    ...
    if request.user.is_authenticated:
        # Auto-provision canteen admin mapping if email matches a college admin
        try:
            user_email = (request.user.email or '').lower().strip()
            if user_email:
                # If no active mapping exists, create one based on College.admin_email
                if not CanteenStaff.objects.filter(user=request.user, is_active=True).exists():
                    matching_college = College.objects.filter(admin_email__iexact=user_email).first()
                    if matching_college:
                        CanteenStaff.objects.get_or_create(
                            user=request.user,
        ...
        # Role-based default when next is root or missing
